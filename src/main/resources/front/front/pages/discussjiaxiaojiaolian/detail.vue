...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjiaxiaojiaolianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjiaxiaojiaolianCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjiaxiaojiaolian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussjiaxiaojiaolian');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjiaxiaojiaolian', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjiaxiaojiaolian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussjiaxiaojiaolian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjiaxiaojiaolian', this.detail);
				this.$utils.msg('审核成功');
