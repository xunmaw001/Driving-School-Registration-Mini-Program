<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="考试科目" prop="kaoshikemu">
          <el-select :disabled="ro.kaoshikemu" v-model="ruleForm.kaoshikemu" placeholder="请选择考试科目">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="考试类型" prop="kaoshileixing">
          <el-select :disabled="ro.kaoshileixing" v-model="ruleForm.kaoshileixing" placeholder="请选择考试类型">
            <el-option
                v-for="(item,index) in kaoshileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="考试类型" prop="kaoshileixing">
	      <el-input v-model="ruleForm.kaoshileixing"
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="报名须知" prop="baomingxuzhi">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="报名须知"
                  v-model="ruleForm.baomingxuzhi" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.baomingxuzhi" label="报名须知" prop="baomingxuzhi">
                    <span>{{ruleForm.baomingxuzhi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="其他说明" prop="qitashuoming">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.qitashuoming" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.qitashuoming" label="其他说明" prop="qitashuoming">
                    <span v-html="ruleForm.qitashuoming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kaoshikemu = true;
          ...
          if(o=='kaoshileixing'){
            this.ruleForm.kaoshileixing = obj[o];
	    this.ro.kaoshileixing = true;
            continue;
          }
          if(o=='baomingxuzhi'){
            this.ruleForm.baomingxuzhi = obj[o];
	    this.ro.baomingxuzhi = true;
            continue;
          }
          if(o=='fabushijian'){
	    ...
	    this.ro.fabushijian = true;
          ...
          if(o=='qitashuoming'){
            this.ruleForm.qitashuoming = obj[o];
	    this.ro.qitashuoming = true;
            continue;
          }
          if(o=='tupian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.kaoshileixingOptions = "A1,A2,B1,B2,C1,C2".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jiaxiaokaoshi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.qitashuoming = this.ruleForm.qitashuoming.replace(reg,'../../../springboot22c8q/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaxiaokaoshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaxiaokaoshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
